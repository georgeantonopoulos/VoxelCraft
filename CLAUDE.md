...
- Woodwork: `vc-audio-woodwork` { kind: saw|sawDone|split|splitDone } → `ambience.woodwork()` (rasping push/pull saw strokes, a trunk creaking apart, the crack of a log splitting). Peaks ~0.15-0.33 vs a footstep's ~0.1 (measured with debugCapture).
...
- Title screen lists saved worlds (`src/state/savedWorlds.ts`, localStorage `vc-worlds-v1`): Continue for the most recent, the others below, each named from its seed (`worldNameFor`). Each world keeps its own inventory (`src/state/inventoryPersistence.ts`, `vc-inventory-v1-<seed>:<type>`, bound in App on entry); returning to the title also clears dropped/thrown items. Campfires, placed torches, planted flora and dropped items persist per world (`src/state/worldObjectsPersistence.ts`, `vc-objects-v1-<seed>:<type>`): restored once `gameStarted` (colliders solid), saved at live physics positions (`physicsItemBodies`) on change, every 8 s, on unload, and flushed before the stores are reset when leaving a world. Letting a world go erases its IndexedDB rows (`<seed>:<type>:` prefix) and its per-seed Grove progress and builds unless another listed world shares the seed.
...
- **Biome climate** (`BiomeManager.getClimate`, single source for terrain shape, biomes, groves, vegetation): the climate maps vary over 1-2 km, so lookups are domain-warped (`climateLookup`: ~110 m bends at 400 m, ~26 m wiggles at 90 m) with a finer detail layer on temperature/humidity; without it every border and coastline was a ruler-straight line at walking scale. Beaches follow the real water line (surface within WATER_LEVEL-4..+2.2: sea shores, river banks, lake edges); `getBiomeAt` gets column heights from terrainShape via `setSurfaceHeightProvider` (cached per metre). Grass reads the biome at a per-blade jittered spot so edges are ragged. Check shapes with `?mode=map`. Each land type is a `CLIMATE_PRESETS` entry (temperature centre, latitude pull, spread, humidity shift/floor, patch size), not special cases; Root Hollow groves form where the climate is mild *for that land* (near its preset centre), so every land has hollows (`generationInvariants.test.ts`). Trees: `BiomeManager.getTreeCover` gives a continuous chance per 4 m cell from the climate (no density step at borders), forest patches/clearings multiply it (uniform-mapped noise), cells roll with `hash01`, and over the 32-per-chunk cap trees are thinned by random rank (stopping in row order left the far rows of dense chunks bare). Sticks fall under every wood-bearing tree (by tree kind; cacti none), visiting trees in seeded random order. Lumina flora grows on dark floors: glowstone/obsidian in the Lumina Depths, or in the Sky Archipelago (no depths) floors roofed over by another island and open to the side (not sealed bubbles). `generationInvariants.test.ts` checks every land has sticks and flora within reach. Caves: three styles (winding grass tunnels, broad desert galleries, tight cold fissures) whose SDFs are blended by column climate (`caveStyleWeights`); picking one style per voxel biome ended tunnels in flat walls under every biome border.
...
- **Trees** (`src/features/flora/trees/`): `treeGrowth.ts` grows species skeletons (gravitropism, curvature noise, whorls for pines, buttress roots) into tapered bark tubes with parallel-transport frames, plus alpha-tested leaf cards with crown-shaped normals. `leafAtlas.ts` draws the leaf textures on a canvas at load. `TreeGeometryFactory` caches per (type, variant, LOD) and keeps the old interface (aBranchDepth/Axis/Origin, aLeafRand). Leaf cards need `customDepthMaterial` (TreeLayer) for cut-out shadows. Root flares are short thick roots that leave the trunk low and end ~0.45 m underground (long straight roots from high up read as stilts and black claws). Keep colliders to <= ~5 per tree (trunk pieces + thickest limbs). Both LODs grow the same skeleton and the same leaf cards (low only drops twig tubes and ring sides), and far crowns fade by each tree's own distance in the leaf shader, never by chunk LOD tier (tier steps made whole chunks of trees change shape and lose leaves at once; `treeGrowth.test.ts`). Bark noise is mapped in real distances (arc length around, length along the branch), never the raw angle (twigs smeared). Leaf atlas paints every leaf inside the canvas with a margin (`fitLeaf`/`inside`), or cards show straight cuts. Bark colours live in TreeLayer and FallingTree: keep them in step.
...
- **Wildlife** (`src/features/creatures/wildlife/`): `wildlifeSim.ts` is pure behaviour (bird flocks: boids, landing/feeding/take-off; deer herds: graze/walk/alert/flee, avoid water; fish schools: cruise/dart/leap; rootling guide: wait/lead/arrive/burrow). `WildlifeManager.tsx` spawns groups 35-80 m out by habitat, scales populations with world vitality, and renders one InstancedMesh per species; `wildlifeMeshes.ts` builds the meshes and animates rig parts in the vertex shader (per-instance `aAnim`). Terrain queries go through a cached 1 m height grid. Rootlings lead to the Lumina compass target and grant the `creaturesGuided` stat. The rootling paces the player (strolls 3.4 m/s, runs up to max(9, 1.3x player speed) when they close within ~7 m or pass it; the run is a bounding gait via the `aAnim.z` channel) and stands on the physics terrain every frame (`surfaceAt` ray; the 1 m analytic grid sank it into slopes). Its limbs are lofted from joints inside the body and pivot there. Debug: `window.__wildlife.summary()`. Tests: `wildlife.test.ts`.
...
- **Shader warm-up covers late arrivals**: `SceneWarmup` compiles only what is visible at load. Things that first appear mid-play (campfire, torch, logs/planks, flying items, the hand) are mounted far below the world during the warm-up and removed after. Never hide the held torch with `visible=false` (it is parked below the view instead): hidden objects are skipped, and the first light-up stalled ~250 ms. It polls programs itself instead of `gl.compileAsync`: three's version throws (and never resolves) when a material is disposed mid-wait, which terrain layers do during load.
- **Never call methods on a possibly-removed Rapier body**: a RigidBody whose `key` changes (planting a stick, anchoring a stone, placing a log) is a new body; the old one is gone. `translation()` on it panics inside Rapier and poisons the world (every later step throws "recursive use of an object ... aliasing in rust": the game froze after placing a second stick). Registries store getters (`physicsItemBodies`) or re-register on remount, and readers check `body.isValid()`. A try/catch does not help: the panic has already poisoned the world.
- **Constant light count**: never mount/unmount or toggle `visible` on lights during play; dim them to intensity 0 instead (TorchTool, FirstPersonTools). Any change in the number of visible lights recompiles every lit shader. `SceneWarmup` precompiles the scene once after load.
...
- **Pickups** fly the real item (`ItemPickupFlight`: lift, arc to the hotbar, shrink) from `useItemPickup` effects (`item` or `tool`). Torches and flora-bound tools fly as their light-free (`isThumbnail`) versions: their full versions mount raw point lights. Leaf hits drop drifting leaves (ImpactFX `leaf`, low gravity) to the tree's foot.
- **Saw, logs and building** (built fresh 2026-09, inspired by the old `saw` branch): a Flint saw is three flakes on the `edge_1..3` slots (`canSaw`, `SAW` action, push-and-draw stroke). Felled trees (`FallingTree`, userData `fallen_tree`, `fallenTreeBodies`) stay 10 min (max 8); sawing emits `vc-tree-sawn` and VoxelTerrain splits the trunk into logs along its axis. `LogStore` is the one source for loose/carried/placed logs (`LogsLayer`, `logBodies`); Q carries/sets down (`useItemPickup`), carrying slows the walk and blocks tools; `BuildPreview` shows the ghost (upright post / lying log, wheel or R) and places on right click (`vc-log-place-request`). Aiming a lying log near the end of a placed lying log turns the corner (crosses at 90°, notched half a log up); planks lie flat, snap edge to edge, and tilt ~30° for pitched roofs in the R mode. Placed and loose logs persist per seed (`vc-logs-v1-<seed>`; loose ones at their physics pose, snapshotted on store change, every 8 s and on unload; a carried log saves as set down at the player's feet). Saves are `{ gen, logs }`; when GEN_VERSION changed since (or an old bare-array save), `settleLogs` re-seats each build (touching placed logs) as one piece on the new ground via `getHeightAt`, and lifts loose logs out of risen ground (`settleLogs.test.ts`). Debug: `window.__logStore`.
- **Weather and shelter** (the reason to build): `WeatherDirector` cycles clear (7-15 min) / gathering / rain (2.5-4 min) / clearing and writes `uOvercast`/`uRain` (sky greys, clouds thicken, sun softens, open ground gets wet, `RainFX` streaks, rain bed + taps in ProceduralAmbience). `isSheltered()` (features/environment/logic/shelter.ts) = placed log/plank or rock straight above. `DousingDirector`: fires and placed torches in the open go out after ~25 s of rain (hiss, steam; fire leaves 2 sticks + stone, torch a stick); covered ones keep burning. Debug: `window.__weather.set('rain')`.
- **Strike timing** (FirstPersonTools + useTerrainInteraction): a press starts the swing (wind-up, blow, recover; styled per action CHOP/DIG/SMASH) and the hit is applied `STRIKE_CONTACT_MS` later, when the blow lands; one strike per press. The swing rotates about screen axes around a grip near the handle butt (adding to the pose's Euler angles turned about the wrong axes). Freeze a phase with `window.__vcSwingHold = { t, style }`.
...
- River current (`logic/waterFlow.ts`): rivers are the n = 0 lines of the signed river noise (`riverSignedAt`, shared with `shapeColumnHeight`), so the flow runs along the perpendicular of its gradient. A 9x9 per-chunk flow texture (every 4 m; direction + strength, fading to the valley rim, none in open sea or deeper than ~5 m) drives streaked ripples and foam flecks advected downstream with two phase-shifted copies. Pure function of world position: chunk borders agree (`waterFlow.test.ts`). The direction is consistent along a river but not tied to downhill (all water is at sea level).
...
- Debug: `window.__waterDebug(n)` (1 depth, 2 seabed bound, 3 contour stripes, 4 river current, 0 normal); `window.__vcDebug.teleport(x,y,z)` / `.look(yaw,pitch)` for browser checks without pointer lock.
