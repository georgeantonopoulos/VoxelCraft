...
- Title screen lists saved worlds (`src/state/savedWorlds.ts`, localStorage `vc-worlds-v1`): Continue for the most recent, the others below, each named from its seed (`worldNameFor`). Each world keeps its own inventory (`src/state/inventoryPersistence.ts`, `vc-inventory-v1-<seed>:<type>`, bound in App on entry); returning to the title also clears dropped/thrown items. Letting a world go erases its IndexedDB rows (`<seed>:<type>:` prefix) and its per-seed Grove progress and builds unless another listed world shares the seed.
