/**
 * Terrain chunk pipeline: everything a terrain worker computes for a chunk,
 * as pure functions (no postMessage, no IndexedDB). The worker only routes
 * messages; benchmarks and tests call these directly so they measure and
 * verify exactly what ships.
 */
import { TerrainService } from '@features/terrain/logic/terrainService';
import { generateMesh, generateWaterSurfaceMesh, HumidityConfig } from '@features/terrain/logic/mesher';
import { MeshData } from '@/types';
import type { ChunkModification } from '@/state/WorldDB';
import { treeVariant, treeRotationY } from '@features/flora/logic/treeInstance';
import { BiomeManager } from './BiomeManager';
import { getVegetationForBiome } from './VegetationConfig';
...
export type GrownTreeInfo = { worldX: number; worldZ: number; ageMs: number };
...
export interface RemeshRequest {
    ...
    /** Only materials/wetness changed (simulation, humidity): keep the collider. */
...
/** Full GENERATE work for one chunk (generation, vegetation, light, mesh, textures). */
export function buildGeneratedChunk(
    cx: number,
    cz: number,
    modifications: ChunkModification[],
    grownTrees: GrownTreeInfo[]
): PipelineResult {
    ...
    if (isEmpty) {
        ...
        return { payload: emptyResponse, transfers: [
...
/** REMESH after an edit: relight and remesh the given voxels. */
export function buildRemeshedChunk(request: RemeshRequest, grownTrees: GrownTreeInfo[]): PipelineResult {
    const { density, material, wetness, mossiness, key, cx, cz, version, floraPositions, materialOnly = false } = request;
