...
export class TreeGeometryFactory {
    ...
    private static generateTree(type: TreeType, variant: number) {
        ...
        // Deterministic RNG for the template (variant enables multiple stable templates per type)
        ...
        let BRANCH_PROB = 0.7;
        let BASE_LENGTH = 2.0;
        let JUNGLE_TRUNK_DEPTH = 4; // How many levels stay as straight trunk
        let JUNGLE_CANOPY_WIDTH = 2.0; // Horizontal widening near leaf tips
        ...
        if (type === TreeType.PINE) {
            ...
            // Variation knobs (deterministic per variant):
            // - Some trees are emergent giants, others are shorter canopy fillers.
            // - Canopy spread/width and trunk straightness vary to avoid repetition.
            MAX_DEPTH = 6 + Math.floor(rand() * 3); // 6..8 depth
            LENGTH_DECAY = 0.85;
            RADIUS_DECAY = 0.65 + rand() * 0.10;    // 0.65..0.75 thick trunks
            ANGLE_BASE = (65 + rand() * 30) * (Math.PI / 180); // 65..95 degrees
            BASE_LENGTH = 3.8 + rand() * 3.4;       // 3.8..7.2 initial trunk height
            BRANCH_PROB = 0.55 + rand() * 0.25;     // 0.55..0.8 branching richness
            JUNGLE_TRUNK_DEPTH = 2 + Math.floor(rand() * 2); // 2..3 straight levels
            JUNGLE_CANOPY_WIDTH = 1.6 + rand() * 0.8; // 1.6..2.4 widening at tips
        }
        ...
        const addLeafClump = (pos: THREE.Vector3, scaleMul: number) => {
            ...
            const lScale = (0.5 + rand() * 0.5) * scaleMul;
        ...
        while (stack.length > 0) {
            ...
            if (seg.depth < MAX_DEPTH) {
                ...
                if (type === TreeType.PALM) {
                    ...
                    // Force straight trunk for the first few levels (variant-controlled)
                ...
                for (let i = 0; i < numBranches; i++) {
                    ...
                    if (type === TreeType.JUNGLE) {
                        // If it's the trunk (depth < JUNGLE_TRUNK_DEPTH), force it straight up
                        if (seg.depth < JUNGLE_TRUNK_DEPTH) {
                    ...
                    if (type === TreeType.JUNGLE && seg.depth >= MAX_DEPTH - 1) {
                        ...
                        newScale.x *= JUNGLE_CANOPY_WIDTH; // Wider horizontally (variant-controlled)
                    ...
                    // Jungle Canopy Volume: add extra leaf clumps along upper branches
                    // so nearby trees merge into a leafy ceiling rather than sparse tips.
                    if (type === TreeType.JUNGLE && seg.depth >= MAX_DEPTH - 3) {
                        const canopyChance = seg.depth >= MAX_DEPTH - 1 ? 0.9 : 0.55;
                        if (rand() < canopyChance) {
                            const clumps = rand() < 0.6 ? 1 : 2;
                            for (let c = 0; c < clumps; c++) {
                                const tAlong = 0.25 + rand() * 0.65; // 25%..90% along the new branch
                                const along = new THREE.Vector3(0, 1, 0)
                                    .applyQuaternion(newQuat)
                                    .multiplyScalar(newScale.y * tAlong);
                                const leafPos = newPos.clone().add(along);
                                const scaleMul = 0.9 + rand() * 0.8;
                                addLeafClump(leafPos, scaleMul);
                            }
                        }
                    }
