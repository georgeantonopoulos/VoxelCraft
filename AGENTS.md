...
### 7. Recent Findings
- 2026-XX-XX: Synced terrain data contract to the optimized material attributes. `MeshData`/`ChunkState` now expose `matIndices`/`matWeights` (with legacy fallbacks), workers transfer those buffers, and `VoxelTerrain` maps them so `ChunkMesh` receives valid attributes. Builds show terrain geometry renders again. Verified with `npm run build` and `npm run dev` (dev served on 127.0.0.1:3000 and was stopped after readiness timeout).
- 2026-XX-XX: Fixed GL attribute type mismatch that caused invisible terrain (`glDrawElements` errors). `ChunkMesh` now binds material indices as float attributes to match the shader’s `vec4` input, preventing WebGL type mismatches. Verified with `npm run build` and `npm run dev` (dev served on 127.0.0.1:3000 and was stopped after readiness timeout).
