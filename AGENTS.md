...
- **Physics Raycast Filter and userData**: When filtering raycasts in Rapier (e.g., `world.castRay(..., filterFn)`), ALL physics bodies must have explicit `userData={{ type: '...' }}` set. Without it, `collider.parent()?.userData?.type` returns `undefined`, which can pass filters unexpectedly. The **player's RigidBody** is particularly problematic - without `userData={{ type: 'player' }}`, the player capsule will be hit by raycasts meant for other physics objects (fallen trees, logs, stones). See `src/features/player/Player.tsx` and `src/features/terrain/hooks/useTerrainInteraction.ts`.
- **Rapier Handle Registries for Dynamic Physics Objects**: `@react-three/rapier` doesn't reliably expose `userData` via `collider.parent()?.userData` for all RigidBody types (especially dynamic bodies like FallingTree). Use a global Map registry keyed by **collider handle** (not RigidBody handle) to store userData. Raycast returns `collider.handle` directly, making collider-based registries more reliable. See `src/features/flora/components/FallingTree.tsx` (`fallingTreeColliderRegistry`).
...
- 2026-09-02: Made complex-terrain collision use the exact rendered mesh and lifted rock shards above the queried terrain surface. Verified focused physics tests, build, unit suite retry, and four settled native-browser views while moving/jumping across slopes.
