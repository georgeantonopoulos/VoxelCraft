...
- 2025-11-26: Restored `flat` qualifiers on tri-material varyings (`vMaterial1/2/3`) in `TriplanarMaterial` so material IDs stay discrete while weights interpolate. This prevents BRDF lookup errors that caused grass/stone seams when IDs interpolated across triangles.
...
- 2025-11-26: Enhanced fragment shader dithering in `TriplanarMaterial.tsx` to mask shard edges. Switched from low-frequency warp noise (0.09 scale) to high-frequency edge noise (1.5 scale) with stronger distortion (0.5 multiplier) to break up straight transition lines using Triple-A technique. The scrambled weights create organic, broken-up transitions between materials.
