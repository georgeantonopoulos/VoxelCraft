...
- 2025-11-25: Flora placement via `KeyE` failed because the raycast filter only checked the hit mesh (child) for `userData.type === 'terrain'`. Added ancestor check so terrain hits are detected even when the flag is on the parent `RigidBody`.
- 2025-11-25: DIG now harvests placed flora (Raycast hits `userData.type === 'flora'` and re-adds to inventory). Flora placement raycast now targets only terrain meshes (no recursion) to reduce brief freeze when pressing `E`.
- 2025-11-25: Restored `KeyE` placement when limiting raycast targets—terrain detection now walks parents before adding a mesh to the raycast list; falls back to full scene if none found.
...
- 2025-11-26: Fixed "Interpolated Integer IDs" artifact using smooth material interpolation in `TriplanarMaterial.tsx`. Replaced hard `floor()` snapping with `getSmoothMaterial()` helper that treats material IDs as gradients. When IDs interpolate (e.g., Dirt 3.0 to Grass 4.0), the function calculates colors for both materials and mixes them based on the decimal part using `smoothstep(0.2, 0.8, t)` for non-linear blending. Adds subtle noise (`vWorldPosition * 0.1`) to break up perfect straight-line transitions. This creates buttery-smooth material transitions (e.g., yellowish-green between Grass and Sand) instead of hard cuts.
- 2025-11-26: Fixed "Material Slot Swapping" artifact in `mesher.ts`. Previously, materials were sorted only by weight, causing the same material to appear in different slots (Slot 1 vs Slot 2) at different vertices, creating hard edges. Implemented canonical ordering: first sort by weight to get top 3 candidates, then sort those by Material ID (ascending) to ensure consistent slot assignment. Now if both Grass (ID 4) and Dirt (ID 3) are present, Dirt will always be in Slot 1 and Grass in Slot 2, regardless of weight. This creates seamless gradients instead of hard cuts at material transitions.
- 2025-11-26: Fixed "Shadow Acne" (shadow striping) artifact in `LuminaFlora.tsx`. Point lights casting shadows on curved/organic terrain surfaces caused rhythmic stepped bands due to precision errors in shadow map depth comparisons. Added `shadow-bias={-0.0005}` and `shadow-normalBias={0.03}` to the pointLight component. The normal bias pushes shadows slightly along surface normals, which is critical for curved geometry. Also optimized shadow map size to `[512, 512]` for performance (point light shadows require 6 renders per light for cubemap). Note: If many LuminaFlora instances are present, consider disabling `castShadow` on them and relying on the Sun's directional light shadows instead.
